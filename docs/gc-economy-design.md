...
- **金方块线（进度货币）**：产出全部来自通关 / 成就 / **一次性**上榜，消耗唯一出口是解锁关卡。
...
| G6 | 首次上榜 | +1 | **闯关榜**（复合键）生平首次上榜 | 一次性（本地标记已领） |
...
| 风险 3 · 金方块被稀释 | 进度货币被广告 / 金币污染 | 铁律 1 + **G5 写死 +8/+12** |
...
| 缺口 | 初稿问题 | 修订 |
| --- | --- | --- |
| 周期榜分流 | 增加认知与维护成本 | 取消周榜/月榜，只保留生涯总榜 |
...
| `js/scenes/rank-scene.js` + `cloudfunctions/rank` | **去掉** classic/timed/marathon；主榜上报 `(clearedCount, linesSum, piecesSum, timeSum)`；好友总榜 + 全服 Top 20 |
