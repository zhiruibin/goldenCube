...
4. **创建数据库集合**
   ...
   - 新建 `player_saves`：用户核心云存档，建议权限设为仅云函数可读写。
...
| `stageProgress` | object | 按关卡 ID 保存的逐关最佳成绩，用于云存档恢复 |
...
### 4. `syncStageProgress` 同步逐关云存档

客户端启动时上传本地逐关最佳成绩，云端按“消行少 → 用块少 → 用时短”合并后返回完整存档。每次通关的 `submitScore` 也会同时写入当前关卡，不增加额外请求。

### 5. `pullPlayerSave` / `pushPlayerSave` 核心云存档

保存金方块、登录徽章、工坊、成就、皮肤、广场进度、无尽模式与设置等核心本地数据。使用 `revision` 防止旧设备覆盖新存档。
